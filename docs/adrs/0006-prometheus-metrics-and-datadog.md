...
- `datadog-agent`'s bootstrap image (before `deploy-datadog-agent` ever pushes the real one) is `gcr.io/google-containers/pause:3.2`, not `api`'s `hello`. Cloud Run rejects more than one container with an exposed port per revision, so the sidecar can't dodge a collision with its own `ports` block — and without one, `hello` falls back to a hardcoded 8080 anyway. `pause` binds nothing.
