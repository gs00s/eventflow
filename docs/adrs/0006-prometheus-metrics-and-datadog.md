...
- `google_cloud_run_v2_service` declares both containers (`api`, `datadog-agent`) with `name` attributes, no `depends_on`/`startup_probe` between them. First attempt added `depends_on = ["datadog-agent"]` on `api` gated on the Agent's `startup_probe` (`GET :5555/live`) — broke production: `terraform apply` runs _before_ `deploy-datadog-agent` pushes the real image, so on the first rollout the sidecar was still the generic `hello` placeholder, which doesn't serve that path. The probe could never pass, and `api` deadlocked waiting on it. Pino's `reconnect`/`recovery` already tolerate the Agent not being up yet, so no ordering guarantee is worth this risk.
