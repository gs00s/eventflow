...
**Database**: schema (user/session/account/verification tables) generated via `npx @better-auth/cli generate` targeting Drizzle, migrated the same way as `speakers` (`drizzle-kit`).
...
**Cookies/CORS topology**: decided — same-origin via reverse proxy, in both dev and production, so default `SameSite=Lax` cookies work with no `trustedOrigins`/`crossSubDomainCookies` config and no Safari/ITP cross-site-cookie risk. The API sets a global `api` prefix (`app.setGlobalPrefix('api')`), so both environments reach it at `/api/*` behind the same frontend origin.
