...
**Secrets**: `DATABASE_URL` and `BETTER_AUTH_SECRET` in Secret Manager, mounted into Cloud Run. Terraform manages both secret containers only, never their values — both are added by hand. `DATABASE_URL` was originally planned as a Terraform-populated value (from the Neon provider's output); moved to manual to keep a single, consistent story for both secrets rather than one Terraform-managed and one hand-managed. The deploy pipeline's migrate/seed step also reads `DATABASE_URL` straight from Secret Manager (`gcloud secrets versions access`), not from a Terraform output — Terraform never re-exposes a secret value once it's not the one setting it. `CORS_ORIGIN`/`BETTER_AUTH_URL` are plain env vars pointed at the Hosting URL; no `env.ts` changes needed. `PORT` is not one of them — Cloud Run reserves that name and injects it automatically based on the container's configured port, an explicit `PORT` env var is rejected outright.
...
- Migrate and seed run immediately after, while the old revision still serves — requires an expand/contract pattern (additive first) since old code must tolerate the new schema during the overlap. Not run at container startup, to avoid concurrent instances racing to migrate. Seeding is idempotent (`onConflictDoNothing()`), safe to run every release.
- Backend (`deploy-api`) and frontend (`deploy-web`) then run **in parallel**, both depending only on the migrate/seed step — not on each other. Originally sequenced (frontend strictly after backend, so a new frontend could never be live before the API it expects), but that ordering assumed a single release routinely ships coupled fullstack changes. In practice, frontend and backend work is ticketed independently, so a given release rarely has that tight a coupling — not worth the slower serial pipeline for the common case. `deploy-api`: build a new multi-stage `Dockerfile` for `apps/api` (handles the pnpm workspace), tag with the git tag, push to Artifact Registry, `gcloud run deploy`. `deploy-web`: `vite build`, `firebase deploy --only hosting`.
