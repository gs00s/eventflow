...
- ~~`push` to `main`: build only (post-merge sanity check)~~ — dropped per [ADR 0004](./0004-versioning-and-deployment-pipeline.md): redundant with `pr.yml`'s pre-merge build and the release/deploy pipeline's own build step, which now also runs on push to `main`.
